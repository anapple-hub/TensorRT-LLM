...
RmsnormPlugin::RmsnormPlugin(float eps, nvinfer1::DataType type)
    ...
    , mType(type)
{
    TLLM_CHECK_WITH_INFO((getSMVersion() >= 80), "Unsupported data type, pre SM 80 GPUs do not support bfloat16");
}
...
RmsnormPlugin::RmsnormPlugin(const void* data, size_t length)
{
    ...
    TLLM_CHECK_WITH_INFO((getSMVersion() >= 80), "Unsupported data type");
}
